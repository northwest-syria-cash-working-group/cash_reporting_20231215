...
author: "This is a PDF of an online report. Click [here](https://northwest-syria-cash-working-group.github.io/cash_reporting_20231215/)
  to access it"
...
progress_line %>% 
  ...
  # We can probably alter the subtitle sooner of later, since earthquake reporting is becoming less and less important 
...
This is a static version of the original interactive plot, which can be accessed [here](https://northwest-syria-cash-working-group.github.io/cash_reporting_20231215/#map-of-activities).
...
cbr %>% 
  ...
  # Big assumption you're making here, but literally all of them have the word "camp" in their names
...
This is a static version of the original interactive plot, which can be accessed [in this section](https://northwest-syria-cash-working-group.github.io/cash_reporting_20231215/#cash-response-actors-and-cash-disbursed).
...
This interactive table does not work in a PDF. Please access it online [here](https://northwest-syria-cash-working-group.github.io/cash_reporting_20231215/#interactive-reference-table-at-sub-district-level).
...
1. The CWG has adopted a new 4Ws template for monthly reporting, aiming to improve the documentation of the earthquake response efforts. **The reporting deadline is set on the 15th of each month.** In February `r filter(month_reporting, month == 2) %>% pull(partners)` partners reported their achievements, In March `r filter(month_reporting, month == 3) %>% pull(partners)` partners have reported their achievements, with `r filter(month_reporting, month == 4) %>% pull(partners)` partners having reported in April, in May `r filter(month_reporting, month == 5) %>% pull(partners)` partners have reported. `r filter(month_reporting, month == 6) %>% pull(partners)` partners have reported in June, with `r filter(month_reporting, month == 7) %>% pull(partners)` partners have reported in July, while `r filter(month_reporting, month == 8) %>% pull(partners)` reported in August, with `r filter(month_reporting, month == 9) %>% pull(partners)` reported in September, with `r filter(month_reporting, month == 10) %>% pull(partners)` reported in October,  with `r filter(month_reporting, month == 11) %>% pull(partners)` reported in November, and `r filter(month_reporting, month == 12) %>% pull(partners)` reported in December. If no MPC activities were implemented, please inform the CWG as well.
