...
```{r progress-line, fig.height=6}
  # This is the messiest chunk in the whole document. could it be fixed? 
...
currency_table <- cbr %>% 
  filter(project_status == "Completed") %>% 
    group_by(currency) %>% 
      summarise(total_usd = sum(total_usd, na.rm = TRUE)) %>%
  # doing this entirely for rounding reasons, because of FX rate, there's one extra dollar
